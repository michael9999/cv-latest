...
function bp_ktr_send_high_five_notification( $to_user_id, $from_user_id ) {
	global $bp;
	...
	if ( 'no' == get_user_meta( (int)$to_user_id, 'notification_ktr_new_high_five', true ) )
		return false;
	...
	$sender_highfive_link = site_url( BP_MEMBERS_SLUG . '/' . $sender_ud->user_login . '/' . $bp->ktr->slug . '/view-orders' );
	...
	$subject = '[' . get_blog_option( 1, 'blogname' ) . '] ' . sprintf( __( '%s high-fived you!', 'bp-ktr' ), stripslashes($sender_name) );
...
---------------------
', 'bp-ktr' ), $sender_name, $sender_name, $sender_profile_link, $sender_name, $sender_highfive_link );

	$message .= sprintf( __( 'To disable these notifications please log in and go to: %s', 'bp-ktr' ), $reciever_settings_link );

	// Send it!
	wp_mail( $to, $subject, $message );
}
add_action( 'bp_ktr_send_high_five', 'bp_ktr_send_high_five_notification', 10, 2 );
