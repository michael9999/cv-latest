		...
		if ($this->path) {
			...
			foreach ($files as $ind => $path) {
				...
				if ($this->type == 'url'){
					...
					if (preg_match('%\W(zip)$%i', trim($path))) {		
					...
					} elseif (preg_match('%\W(csv)$%i', trim($path))) {																				
						...
						if (empty($this->large_import) or $this->large_import == 'No') {	
							$fileInfo = (preg_match('%\W(gz)$%i', trim($path))) ? pmxi_gzfile_get_contents($path) : pmxi_copy_url_file($path, true);
				...
				if (empty($this->large_import) or $this->large_import == 'No'){
					...
					$logger = create_function('$m', 'echo "<div class=\\"progress-msg\\">$m</div>\\n"; $_SESSION[\'pmxi_import\'][\'log\'] .= $m."\\n"; flush();');	

					$this->process($xml, ($cron) ? false : $logger, false, $cron );
				}
					...
					foreach ($serialized_meta as $m_key => $values) { 
						...
						{							
							if (strpos($m_key, '[') !== false){
								...
								update_post_meta($pid, $meta_name, array($meta_key => $values[0][$i]));
							}	
							else{
								$current_meta_keys[] = $m_key;
