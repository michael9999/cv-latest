...
//alert("before post");
    //var id_uni = jQuery(event.target).attr('id');
      ...
      jQuery.post("http://www.cv-en-anglais.com/php-mobile/process-support-form.php", {user : support_user, email : support_email, issue : support_issue, customer_type : support_customer_type, description : support_description}, function(data){
       if (data.length>0){
...
//var tick_value = jQuery(event.target).attr('checked');
    //var id_uni = jQuery(event.target).attr('id');
      ...
      jQuery.post("http://www.cv-en-anglais.com/php-mobile/get-form.php", {customer_type : support_customer_type, type1 : type, user : support_user, email : support_email}, function(data){
       if (data.length>0){
         //alert("DATA RETURNED, ok to hereE");
         //$("#search_results").html(data);
         jQuery("#support-form-holder").empty();
