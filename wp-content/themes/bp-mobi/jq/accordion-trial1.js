...
jQuery(document).ready(function(){
    ...
    /* GET LIST OF CHECK BOXES, NON-SPECIFIC QUESTIONS */
...
//jQuery("#wp-accordion").execphp-7.addClass("active");
//
//alert("current user is " + user);
...
//alert("php page executed " + $et)

//alert("test php link clicked");

// Add event to dynamic elements
//<script>$("ul.topnav > li").css("border", "3px double red");</script>
    //jQuery('#questions_trial22').live('click', function(event){
//jQuery('#questions_trial').live('click', function(event){
...
/* eq carries pane number to open */
    ...
    /* -------------------- GET LIST OF UNIVERSITIES TO CHOOSE FROM ---------------------------*/

//alert("about to run methods");
...
// Need to change this as Class space is being used by JQM

//var id = jQuery(event.target).attr('class');

//$("someselection").parent().dostuff

//var id = jQuery(event.target).attr('style');

//var id = jQuery(event.target).parent().attr('style');
...
//          var box_text=jQuery('#uni-list2 :selected').text();
         ...
         // alert("Different uni selected " + box_val + box_text);
          //addUni(user);
...
// --- end delegate
 ...
 /*   jQuery("#specific_questions").live("click", function(event){
    ...
    //var t = jQuery(event.target);
//alert ("specific questions are running");
...
//alert("this the value from height value " + tick_value);
    //var tick_value = jQuery(event.target).attr('checked');
    //var id_quest = jQuery(event.target).attr('id');
    ...
    tick_value = "no";
  //  alert ("value sent to db" + tick_value);
...
//alert("tick value is " + tick_value);
...
//alert("getNs questions running");
      ...
      jQuery.post("http://cv-en-anglais.com/php-mobile/Show_NS_Questions.php", {user_current : user }, function(data){
       if (data.length>0){
...
//jQuery('#landing-page-data').html(data);
...
//el.html(data); //Replace the html in el with the returned data
...
//alert("try: page function");
//jQuery('.non_specific_questions').page();
//alert("just after page function");
//------------------ end apply styles
...
//alert("getS questions running");
      ...
      jQuery.post("http://cv-en-anglais.com/php-mobile/Show_S_Questions.php", {user_current : user }, function(data){
       if (data.length>0){
           //alert("getNs: ok to here");
         //$("#search_results").html(data);
         //jQuery("#results").empty();
...
/* GET NON SPECIFIC QUESTION DATA FROM DATABASE */
    ...
    function getUcas(user){
      ...
      jQuery.post("http://cv-en-anglais.com/php-mobile/Show_Ucas_Questions.php", {user_current : user }, function(data){
       if (data.length>0){
...
/* GET ENGLISH QUESTIONS-LINKS */
    ...
    function getEnglish(user){
      ...
      jQuery.post("http://cv-en-anglais.com/php-mobile/Show_English_Questions.php", {user_current : user }, function(data){
       if (data.length>0){
           //alert("get UCAS: ok to here");
         //$("#search_results").html(data);
...
/* GET ENGLISH QUESTIONS-LINKS */
    ...
    function getBeforeLeaving(user){
      ...
      jQuery.post("http://cv-en-anglais.com/php-mobile/Show_Before_Leaving_Questions.php", {user_current : user }, function(data){
       if (data.length>0){
...
alert("addLink started");

alert("ensure variable data is still present");
...
alert(user + link_name + link_address);
    ...
    //var id_uni = jQuery(event.target).attr('id');
      ...
      jQuery.post("http://www.cv-en-anglais.com/php-mobile/add_link.php", {user1 : user, link_name1 : link_name, link_address1 : link_address}, function(data){
       if (data.length>0){
           //alert("updateSQuestions: ok to here on UPDATE");
         //$("#search_results").html(data);
         //jQuery("#s_questions").empty();
...
alert("running inside: about to retrieve links");
...
//alert("cookie not set after click");
initialValue = "open";
lifeTime = 7;
createCookie(mobVarTest,initialValue,lifeTime);
testTest = readCookie(mobVarTest);
//alert("this is the cookie value " + testTest);
