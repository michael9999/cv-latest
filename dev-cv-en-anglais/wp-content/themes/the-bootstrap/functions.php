...
function the_bootstrap_setup() {
	...
	register_nav_menus( array(
		...
		'buddypress-menu' 	=>	__( 'Buddypress Menu', 'the-bootstrap' )
	) );
...
function the_bootstrap_widgets_init() {
	...
	register_sidebar( array(
		'name'			=>	__( 'Home page widgets', 'the-bootstrap' ),
		'id'			=>	'home',
	...
	register_sidebar( array(
		'name'			=>	__( 'Footer sidebar 1', 'the-bootstrap' ),
		'description'	=>	__( 'First widget area in the footer.', 'the-bootstrap' ),
		'id'			=>	'footer_1',
	...
	register_sidebar( array(
		'name'			=>	__( 'Footer sidebar 2', 'the-bootstrap' ),
		'description'	=>	__( 'Second widget area in the footer.', 'the-bootstrap' ),
		'id'			=>	'footer_2',
		'before_widget'	=>	'<aside id="%1$s" class="widget %2$s">',
		'after_widget'	=>	'</aside>',
		'before_title'	=>	'<h4 class="widget-title">',
	...
	register_sidebar( array(
		'name'			=>	__( 'Footer sidebar 3', 'the-bootstrap' ),
		'description'	=>	__( 'Third widget area in the footer.', 'the-bootstrap' ),
		'id'			=>	'footer_3',
		'before_widget'	=>	'<aside id="%1$s" class="widget %2$s">',
		'after_widget'	=>	'</aside>',
		'before_title'	=>	'<h4 class="widget-title">',
	...
	register_sidebar( array(
		'name'			=>	__( 'Footer sidebar 4', 'the-bootstrap' ),
		'description'	=>	__( 'Fourth widget area in the footer.', 'the-bootstrap' ),
		'id'			=>	'footer_4',
		'before_widget'	=>	'<aside id="%1$s" class="widget %2$s">',
		'after_widget'	=>	'</aside>',
		'before_title'	=>	'<h4 class="widget-title">',
...
function the_bootstrap_register_scripts_styles() {
	...
	if ( ! is_admin() ) {
		...
		wp_register_style(
			'fontawesome',
			get_template_directory_uri() . "/css/font-awesome{$suffix}.css",
			array('tw-bootstrap'),
			$theme_version
		);
...
/*
*	Adds the font-awesome meta box to the pages
*/

add_action( 'add_meta_boxes', 'the_bootstrap_add_custom_box' );
/* Do something with the data entered */
add_action( 'save_post', 'the_bootstrap_save_postdata' );

function the_bootstrap_add_custom_box(){
	add_meta_box( 'custom-menu-icon', __( 'Help Menu Icon', 'the_bootstrap' ), 'the_bootstrap_custom_box_inner', 'page');
}

function the_bootstrap_custom_box_inner($post){
	// Use nonce for verification
	wp_nonce_field( plugin_basename( __FILE__ ), 'the_bootstrap_noncename' );

	// The actual fields for data entry
	// Use get_post_meta to retrieve an existing value from the database and use the value for the form
	$value = get_post_meta( $post->ID, '_custom_icon', true );
	echo '<label for="the_bootstrap_custom_icon">';
	   _e("Font-Awesome icon name", 'the_bootstrap' );
	echo '</label> ';
	echo '<input type="text" id="the_bootstrap_custom_icon" name="the_bootstrap_custom_icon" value="'.esc_attr($value).'" size="25" />';
	echo '<p>Exemple : icon-home</p>';
	echo '<p>Find the icon list on <a href="http://fortawesome.github.io/Font-Awesome/icons/" target="_blank">Font-Awesome</a></p>';
}
function the_bootstrap_save_postdata( $post_id ) {
  ...
  if ( ! isset( $_POST['the_bootstrap_noncename'] ) || ! wp_verify_nonce( $_POST['the_bootstrap_noncename'], plugin_basename( __FILE__ ) ) )
      return;
  ...
  //sanitize user input
  $mydata = sanitize_text_field( $_POST['the_bootstrap_custom_icon'] );

  // Do something with $mydata 
  // either using 
  add_post_meta($post_ID, '_custom_icon', $mydata, true) or
    update_post_meta($post_ID, '_custom_icon', $mydata);
...
function the_bootstrap_print_ie_scripts() {
	...
	<?php 
		$color_start = get_theme_mod('header_color1_start');
		$color_end = get_theme_mod('header_color1_end');
	 ?>

	<style type="text/css">
        .navbar-inner { 
        	background-color: <?php echo $color_start; ?>;
        	background-image: linear-gradient(to bottom,  <?php echo $color_start; ?> 0%,<?php echo $color_end; ?> 100%);
        	background-image: -webkit-linear-gradient(top,  <?php echo $color_start; ?> 0%,<?php echo $color_end; ?> 100%);
        	background-image: -o-linear-gradient(top,  <?php echo $color_start; ?> 0%,<?php echo $color_end; ?> 100%);
        	background-image: -o-linear-gradient(top,  <?php echo $color_start; ?> 0%,<?php echo $color_end; ?> 100%);
        	background-image: -ms-linear-gradient(top,  <?php echo $color_start; ?> 0%,<?php echo $color_end; ?> 100%);
        	filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='<?php echo $color_start; ?>', endColorstr='<?php echo $color_end; ?>',GradientType=0 );
        }
    </style>
...
function the_breadcrumbs (){
	global $post;
	$pid = get_the_ID();
	$trail = '<li class="first"><a href="/">'. __('Home', 'textdomain') .'</a></li>';
	...
	elseif (is_page()) {
		$bcarray = array();
		$pdata = get_post($pid);
		$bcarray[] = '<li class="last">'.$pdata->post_title.'</li><li class="finish">&nbsp;</li>'."\n";
		while ($pdata->post_parent) {
			$pdata = get_post($pdata->post_parent);
			$bcarray[] = '<li class="next"><a href="'.get_permalink($pdata->ID).'">'.$pdata->post_title.'</a></li>';
		}
    ...
    elseif (substr($post->post_type, 0, 3) == 'bp_'){
    	
    	global $bp;
    	
    	$title = null;
    	foreach ($bp->pages as $key => $comp) {
    		if($key == $bp->current_component)
    			$title = $comp->title;
    	}
    	if(isset($title))
    		$trail .= '<li class="next">'.$title.'</li><li class="finish">&nbsp;</li>';
    	else
    		$trail .= '<li class="next">'.get_the_title().'</li><li class="finish">&nbsp;</li>';
    }
	$trail.="";
